...
if ($("#annotations-chart").length > 0) {
    for (var a = [], b = 0; b < 20; ++b) a.push([b, Math.sin(b)]);
        ...
        e.append("<div style='position:absolute;left:" + (g.left + 4) + "px;top:" + g.top + "px;color:#666;font-size:smaller'>خطا بالا</div>"), g = f.pointOffset({
            x: 8,
            y: -1.2
        }), e.append("<div style='position:absolute;left:" + (g.left + 4) + "px;top:" + g.top + "px;color:#666;font-size:smaller'>اندازه گیری واقعی</div>");
...
$(function() {
    ...
    var data4_errors = [0.1, 0.4, 0.2];
        ...
        var data = [
            {color: "#fe80b2", points: data1_points, data: data1, label: "اطلاعات 1"},
            {color: "#fb2e63",  points: data2_points, data: data2, label: "اطلاعات 2"},
            {color: "#fe80b2", lines: {show: true}, points: data3_points, data: data3, label: "اطلاعات 3"},
            {color: "#7e37d8", bars: {show: true, align: "center", barWidth: 0.25}, data: data4, label: "اطلاعات 4"},
